...
d) Finalmente, se ejecuta la función `DNA_to_RNA` sobre cada una de las secuencias de la matriz `secuencias` y se muestra el resultado en cada iteración:

   ```{r}
   # Ejecutar la función DNA_to_RNA sobre la matriz secuencias
   for (i in 1:nrow(secuencias)) {
     adn <- secuencias[i, 1]
     rna <- DNA_to_RNA(adn)
     if (!is.null(rna)) {
...
a) Para definir la función `indice_normalizacion`, es necesario incluir un control de errores que verifique que los parámetros recibidos sean numéricos y positivos. Esta comprobación es importante, ya que el índice solo tiene sentido si se trabaja con valores válidos. Una vez superada esa validación, se calcula el índice de normalización y se devuelve el resultado.
   ...
   indice_normalizacion <- function(lecturas_gen, lecturas_totales) {
     ...
     if (lecturas_gen < 0 || lecturas_totales <= 0) {
       stop("Error: Las lecturas deben ser positivas y el total debe ser mayor que cero.")
     }
...
b) Para comprobar el resultado de la función anterior, se pueden hacer pruebas con los parámetros del enunciado y con otros valores. De este modo, no solo se verifica que el cálculo sea correcto en un caso válido, sino también que el control de errores funcione como debe ante entradas incorrectas.
   ...
   ```
      A partir de los resultados, se comprueba que la función devuelve el índice correcto cuando los valores son válidos y que muestra mensajes de error en los casos no válidos. Por tanto, la función no solo calcula el índice, sino que además controla adecuadamente situaciones que podrían dar lugar a resultados erróneos.
...
a) Para cargar el paquete `datasets`, el conjunto de datos `Theoph` y las librerías necesarias para trabajar con bases de datos en R, se puede utilizar el siguiente código:
   ...
   # Mostrar diez primeras filas del conjunto de datos Theoph
   head(Theoph, 10)
...
d) Para obtener el tiempo en el que ocurre la concentración máxima por paciente, se puede utilizar la siguiente consulta SQL:
   ...
   query_max <- "
   SELECT t1.Subject, t1.Time, t1.conc
   FROM Theoph t1
   WHERE t1.conc = (
     SELECT MAX(t2.conc)
     FROM Theoph t2
   ...
   print(resultado_max)
   ```
    En este caso, se utiliza una subconsulta correlacionada. Para cada fila de la tabla Theoph, la subconsulta obtiene el valor máximo de concentración (**`MAX(conc)`**) del mismo sujeto. Después, la consulta principal selecciona únicamente aquellas filas cuya concentración coincide con ese máximo. De este modo, no solo se identifica la concentración máxima de cada paciente, sino también el tiempo exacto en que se alcanza.
...
b) Para calcular la probabilidad de que al menos 25 de los 30 pacientes con respuesta al tratamiento mejoren, se puede utilizar la distribución binomial con probabilidad de éxito 0.80. Del mismo modo, para el grupo sin respuesta se trabaja con la probabilidad correspondiente a ese caso:
   ...
   cat("Probabilidad de que al menos 25 pacientes con respuesta mejoren:", prob_al_menos_25_respuesta, "\n")

   # Probabilidad de que como máximo 25 de los 80 pacientes que no responden mejoren
   prob_maximo_25_no_respuesta <- pbinom(25, size = 80, prob = 0.15)
   cat("Probabilidad de que como máximo 25 pacientes que no responden mejoren:", prob_maximo_25_no_respuesta, "\n")
   ```
...
c) Para representar la distribución del número de pacientes que mejoran en ambos grupos, se puede utilizar un histograma o una gráfica de barras. A continuación, se muestra cómo hacerlo:
   ...
   simulaciones_no_respuesta <- rbinom(10000, size = 80, prob = 0.15)

   # Representar las distribuciones
   par(mfrow = c(1, 2)) # Para mostrar ambas gráficas lado a lado
   hist(simulaciones_respuesta, breaks = seq(-0.5, 30.5, by = 1), col = "blue", main = "Pacientes con Respuesta", xlab = "Número de Pacientes que Mejoran")
   hist(simulaciones_no_respuesta, breaks = seq(-0.5, 80.5, by = 1), col = "red", main = "Pacientes sin Respuesta", xlab = "Número de Pacientes que Mejoran")
   ```
    En la gráfica se puede ver que el número de pacientes que mejoran es claramente mayor en el grupo que responde al tratamiento, ya que la distribución está centrada en valores más altos. En cambio, en el grupo que no responde, la mayoría de los valores se concentran en números bajos.

    Esta diferencia se explica por la probabilidad de mejora en cada grupo: en los pacientes que responden es alta (0.80), mientras que en los que no responden es bastante menor (0.15). Esto hace que las distribuciones sean muy diferentes entre sí. 
    
    En conjunto, los resultados muestran de forma bastante clara que el tratamiento tiene un efecto notable en los pacientes que responden, mientras que en el resto la mejoría es mucho menos frecuente.
